...
public class Simplex {
    ...
    */
    ...
    public Simplex(float[] zeta, float[][] restricciones) {
        ...
        int numColumnas = restricciones[0].length + restricciones.length;
float[][] tablaInicial = new float[zeta.length + 1][numColumnas];
...
public void imprimirTabla(float[][] tabla) {
    for (float[] fila : tabla) {
        for (float valor : fila) {
            // Usar printf con un ancho de campo de 12 caracteres para cada número
            System.out.printf("%12.2f ", valor);
        }
    ...
    public void operacionesPivote(int indiceMasNegativo){
        float[][] tabla = iteraciones.get(iteraciones.size() - 1).clone();
        float pivote = getPivote(indiceMasNegativo);
        ...
        for (int j = 0; j < tabla[filaPivote].length-1; j++) {
    ...
    public void CalcularRadios(int indiceMasNegativo) {
        float[][] ultimaIteracion = iteraciones.get(iteraciones.size() - 1).clone();
